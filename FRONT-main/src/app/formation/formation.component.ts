    ...
    this.ngZone.run(() => {
      this.formationservice.getFormationsParRH(Number(rhId)).subscribe(
        (data) => {
          ...
          this.formationsPolyvalenceEnAttente = this.formationsPolyvalence.filter(f => 
            !f.valide
          );
...
updateResultat(formationId: number, employeId: number, resultat: string, employe: any) {
  ...
  // Cas normal pour les autres formations (y compris POLYVALANCE)
    ...
    this.confirmationService.confirm({
      message: `Vous avez sélectionné "Programme Complémentaire" pour ${employe.nom} ${employe.prenom}. Souhaitez-vous modifier la formation ?`,
      header: 'Programme Complémentaire',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        // Ouvrir le dialogue de modification
        this.openModificationDialog(this.selectedFormation);
      },
      reject: () => {
        // Enregistrer quand même le résultat sans modifier la formation
        this.formationservice.ajouterResultatFormation(formationId, employeId, resultat).subscribe({
          next: (response) => {
            this.messageService.add({
              severity: 'success',
